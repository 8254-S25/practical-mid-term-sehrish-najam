...
```
scp ~/Desktop/sehrishnajam.txt pi@192.168.86.23:~
```
...
```
pwd>mt.txt with print the current working directory in the mt.txt file if the file already exists it will be over written.
tree -l>>pr.txt will log the tree output of the directory structure to the existing pr.txt file. if the file does not exist new will be created.
```
...
```
sudo mkdir /home/sehrishnajam
sudo chown sehrishnajam:sehrishnajam /home/sehrishnajam
```
